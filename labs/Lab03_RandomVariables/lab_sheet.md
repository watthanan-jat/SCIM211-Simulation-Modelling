...
The notebook provides the summary-table and plotting code. After completing the simulation code, run the provided cells to display:
...
Using seed `211`, continue until you have 1,000 accepted values. The notebook provides the reporting table and histogram code. Complete the sampling and summary calculations, then use the provided code to report:
...
1. 1,000 inverse-transform exponential service times, the non-negative check, summary table, and histogram;
2. the NumPy sample, passed consistency check, and written response; and
3. 1,000 accepted values, the proposal count, acceptance rate, sample mean, and histogram.

The TA may ask you to explain one of the sampling methods. The TA will record the checkpoint during the lab. Continue with Parts D-E.
...
4. run the provided code to create a table of counts, simulated proportions, and average service times by drink; and
...
Let $D$ be the drink category and $T$ the service time. For drink $j$, define

$$
p_j=P(D=j), \qquad \mu_j=E[T\mid D=j].
$$

A drink can create high workload because it is ordered frequently, because it takes a long time to prepare, or both. Its expected contribution to service demand per arriving customer is
...
The contributions from all drink categories add to the overall expected service time:
...
5. Why can the continuous inverse-transform proof not be applied directly to a step-shaped discrete CDF? How does selecting the first cut point at or above `U` solve the problem?
...
Part D defined each category's expected workload contribution as $p_j\mu_j$. Adding these contributions gives the Law of Total Expectation:

$$
\underbrace{p_j\mu_j}_{\text{contribution from category }j}
\quad\Longrightarrow\quad
\underbrace{\sum_jp_j\mu_j}_{E[T]}.
$$

Keep the drink-specific conditional means from Part D. Once the probabilities $p_j$ and conditional means $\mu_j$ are specified, expected service demand can be calculated analytically. No new simulation is required: **simulate when necessary; calculate analytically when the model gives an exact quantity.**
